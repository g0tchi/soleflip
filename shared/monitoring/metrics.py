    ...
    def decorator(func):
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration_ms = (time.time() - start_time) * 1000
                asyncio.create_task(registry.record_value(metric_name, duration_ms, {"status": "success"}))
                return result
            except Exception as e:
                duration_ms = (time.time() - start_time) * 1000
                asyncio.create_task(registry.record_value(metric_name, duration_ms, {"status": "error"}))
                raise
...
def track_counter(metric_name: str, labels: Optional[Dict[str, str]] = None):
    ...
    def decorator(func):
        ...
        def sync_wrapper(*args, **kwargs):
            try:
                result = func(*args, **kwargs)
                asyncio.create_task(registry.record_value(metric_name, 1, {**(labels or {}), "status": "success"}))
                return result
            except Exception as e:
                asyncio.create_task(registry.record_value(metric_name, 1, {**(labels or {}), "status": "error"}))
                raise
...
async def track_operation(operation_name: str, labels: Optional[Dict[str, str]] = None):
    ...
    registry.register_metric(duration_metric, MetricType.HISTOGRAM, MetricUnit.MILLISECONDS, f"{operation_name} duration")
    registry.register_metric(counter_metric, MetricType.COUNTER, MetricUnit.COUNT, f"{operation_name} total")
    
    start_time = time.time()
    try:
        yield
        duration_ms = (time.time() - start_time) * 1000
        await registry.record_value(duration_metric, duration_ms, {**(labels or {}), "status": "success"})
        await registry.record_value(counter_metric, 1, {**(labels or {}), "status": "success"})
    except Exception as e:
        duration_ms = (time.time() - start_time) * 1000
        await registry.record_value(duration_metric, duration_ms, {**(labels or {}), "status": "error"})
