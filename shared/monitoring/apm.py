...
class APMCollector:
    ...
    def record_system_metrics(self, metrics: SystemMetrics):
        ...
        if metrics.cpu_percent > self.high_cpu_threshold:
            self._create_alert(
                "high_cpu",
                {"cpu_percent": metrics.cpu_percent, "threshold": self.high_cpu_threshold},
            )
    ...
    def get_performance_summary(self, minutes: int = 15) -> Dict[str, Any]:
        ...
        return {
            ...
            "health_score": self._calculate_health_score(
                request_stats, database_stats, system_stats, recent_alerts
            ),
        }
    ...
    def _calculate_database_stats(self, db_ops: List[DatabaseMetrics]) -> Dict[str, Any]:
        ...
        slow_queries = sum(
            1 for op in db_ops if op.execution_time_ms > self.slow_query_threshold_ms
        )
        ...
        return {
            ...
            "by_query_type": {
                k: {"count": v["count"], "avg_time_ms": round(v["total_time"] / v["count"], 2)}
    ...
    def _calculate_health_score(
        self, request_stats: Dict, db_stats: Dict, system_stats: Dict, alerts: List[Dict]
    ) -> float:
