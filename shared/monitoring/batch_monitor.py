...
class BatchProcessingMonitor:
    ...
    async def _trigger_alert(self, alert: BatchAlert):
        ...
        metrics.increment_counter(
            "batch_alerts_total", labels={"type": alert.alert_type, "severity": alert.severity}
        )
        ...
        async for session in get_db_session():
            try:
                ...
                total_count = await session.scalar(
                    select(func.count(ImportBatch.id)).where(ImportBatch.created_at >= cutoff_time)
                )
                ...
                successful_count = await session.scalar(
                    select(func.count(ImportBatch.id)).where(
                        and_(
                            ImportBatch.created_at >= cutoff_time, ImportBatch.status == "completed"
                        )
                    )
                )
                ...
                failed_count = await session.scalar(
                    select(func.count(ImportBatch.id)).where(
                        and_(ImportBatch.created_at >= cutoff_time, ImportBatch.status == "failed")
                    )
                )

                # Currently retrying batches
                retrying_count = await session.scalar(
                    select(func.count(ImportBatch.id)).where(ImportBatch.status == "retrying")
                )
                ...
                avg_processing_time = (
                    await session.scalar(
                        select(
                            func.avg(
                                func.extract(
                                    "epoch", ImportBatch.completed_at - ImportBatch.started_at
                                )
                                / 60
                            )
                        ).where(
                ...
                if failure_rate > self.monitoring_thresholds["failure_rate_threshold"] * 100:
                    await self._trigger_alert(
                        BatchAlert(
                            ...
                            severity=(
                                AlertSeverity.HIGH if failure_rate > 30 else AlertSeverity.MEDIUM
                            ),
                            ...
                            metadata={
                                ...
                                "threshold": self.monitoring_thresholds["failure_rate_threshold"]
                                * 100,
                            },
                        )
                    )
                        ...
                        await self._trigger_alert(
                            BatchAlert(
                                ...
                                metadata={
                                    ...
                                    "status": batch.status,
                                    "hours_processing": self.monitoring_thresholds[
                ...
                if (
                    avg_processing_time
                    > self.monitoring_thresholds["max_processing_time_hours"] * 60
                ):
                    await self._trigger_alert(
                        BatchAlert(
                            ...
                            metadata={
                                ...
                                "threshold_minutes": self.monitoring_thresholds[
                ...
                self.logger.info("Batch processing monitoring completed", stats=stats.__dict__)
            ...
            try:
                ...
                if batch.status in ["processing", "retrying"] and batch.started_at:
                    ...
                    if (
                        ...
                        > self.monitoring_thresholds["stuck_batch_threshold_hours"] * 3600
                    ):
                        return BatchAlert(
    ...
    async def get_batch_health_summary(self) -> Dict[str, Any]:
        ...
        async for session in get_db_session():
            try:
                ...
                health_summary = {"overall_status": "healthy", "windows": {}}
