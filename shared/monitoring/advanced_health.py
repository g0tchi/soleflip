...
class AdvancedHealthChecker:
    ...
    async def get_comprehensive_health(self) -> Dict[str, Any]:
        ...
        if (self.last_full_check and 
            (now - self.last_full_check).total_seconds() < self.cache_ttl_seconds):
            ...
            async with db_manager.get_session() as session:
                ...
                if table_count > 0:
                    return HealthCheckResult(
                        ...
                        response_time_ms=response_time,
                        details={"table_count": table_count, "active_connections": active_connections}
                    )
                else:
                    return HealthCheckResult(
                        name="database_readiness",
    ...
    async def _check_essential_tables(self) -> HealthCheckResult:
        ...
        try:
            async with db_manager.get_session() as session:
                ...
                failed_tables = [t for t, status in table_status.items() if status.startswith("error")]
