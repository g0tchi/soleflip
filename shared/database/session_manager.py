...
class SessionStats:
    ...
    def get_stats(self) -> Dict[str, Any]:
        return {
            ...
            "success_rate": (
                (self.total_sessions - self.failed_sessions) / max(self.total_sessions, 1)
            )
            * 100,
...
class SessionManager:
    ...
    async def _cleanup_stale_sessions(self):
        ...
        stale_sessions = [
            session_id
            for session_id, start_time in self.stats._session_start_times.items()
            if start_time < cutoff_time
        ]

        if stale_sessions:
            logger.warning(
                "Found stale sessions", count=len(stale_sessions), session_ids=stale_sessions
            )
    ...
    async def get_session(
        self, autocommit: bool = True, read_only: bool = False
        ...
        try:
            ...
            try:
                ...
                logger.error("Error closing session", session_id=session_id, error=str(close_error))
    ...
    async def execute_with_retry(
        self, operation, max_retries: int = 3, retry_delay: float = 1.0, **session_kwargs
    ):
    ...
    async def bulk_insert(self, model_class, data: list, batch_size: int = 1000):
        ...
        async with self.get_session() as session:
            ...
            await session.commit()
            logger.info(
                "Bulk insert completed", model=model_class.__name__, total_records=len(data)
            )
        ...
        for field, value in filters.items():
            if value is not None:
                if isinstance(value, str) and "%" in value:
                    ...
                    query = query.filter(
                        getattr(query.column_descriptions[0]["type"], field).like(value)
                    )
                else:
                    # Exact match for other values
                    query = query.filter(
                        getattr(query.column_descriptions[0]["type"], field) == value
                    )
        return query
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            async with session_manager.get_session(
                autocommit=autocommit, read_only=read_only
            ) as session:
