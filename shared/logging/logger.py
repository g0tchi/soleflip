...
class DatabaseLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            details = {k: v for k, v in log_data.items() if k not in ["message", "component"]}
        ...
        async def write_log():
            try:
                ...
                async with get_db_session() as db:
                    log_entry = SystemLog(
                        level=level, component=component, message=message, details=details or None
                    )
                    db.add(log_entry)
                    await db.commit()
            except Exception:
...
def setup_logging(
    log_level: str = "INFO", enable_database_logging: bool = True, enable_json_output: bool = None
...
def log_function_call(func):
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.error(
                "Function failed", function=func.__name__, error=str(e), error_type=type(e).__name__
            )
            raise
