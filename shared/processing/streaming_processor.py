...
class DataStreamProcessor:
    ...
    async def process_csv_stream(
        self, file_path: str, batch_id: UUID, source_type: str, encoding: str = "utf-8-sig"
    ) -> ProcessingContext:
        """
    ...
    async def process_json_stream(
        self, file_path: str, batch_id: UUID, source_type: str
    ) -> ProcessingContext:
        """
    ...
    async def _create_csv_stream(
        ...
        async with aiofiles.open(
            file_path, mode="r", encoding=encoding, buffering=self.config.buffer_size
        ) as file:
            # Read header
            ...
            async for line in file:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to parse CSV row", line_number=record_count + 1, error=str(e)
                    )
                    continue
    ...
    async def _create_jsonl_stream(
        self, file_path: str
    ) -> AsyncGenerator[List[Dict[str, Any]], None]:
        ...
        async with aiofiles.open(
            file_path, mode="r", encoding="utf-8", buffering=self.config.buffer_size
        ) as file:
            async for line in file:
                line = line.strip()
                if not line:
                ...
                try:
                    ...
                    if len(current_chunk) >= self.config.chunk_size:
                        ...
                        if record_count % self.config.progress_interval == 0:
                            logger.debug("JSONL streaming progress", records_processed=record_count)

                except json.JSONDecodeError as e:
                    logger.warning(
                        "Failed to parse JSON line", line_number=record_count + 1, error=str(e)
                    )
                    continue
    ...
    async def _create_json_array_stream(
        self, file_path: str
    ) -> AsyncGenerator[List[Dict[str, Any]], None]:
    ...
    async def _chunk_api_stream(
        self, api_generator: AsyncGenerator[Dict[str, Any], None]
    ) -> AsyncGenerator[List[Dict[str, Any]], None]:
        ...
        async for record in api_generator:
            ...
            if len(current_chunk) >= self.config.chunk_size:
                ...
                if record_count % self.config.progress_interval == 0:
                    logger.debug("API streaming progress", records_processed=record_count)
    ...
    async def _estimate_csv_records(self, file_path: str) -> Optional[int]:
        ...
        try:
            ...
            logger.warning("Failed to estimate CSV record count", file_path=file_path, error=str(e))
            return None
    ...
    async def _estimate_jsonl_records(self, file_path: str) -> Optional[int]:
        ...
        try:
            ...
            logger.warning(
                "Failed to estimate JSONL record count", file_path=file_path, error=str(e)
            )
            return None
