...
class SearchParams:
    def __init__(
        ...
        min_price: Annotated[Optional[float], Query(ge=0, description="Minimum price filter")] = None,
        max_price: Annotated[Optional[float], Query(ge=0, description="Maximum price filter")] = None
    ):
        self.query = q
        self.brand = brand
    ...
    def has_filters(self) -> bool:
        ...
        return any([
            self.query, self.brand, self.category, 
            self.status, self.size, self.min_price, self.max_price
        ])
...
async def validate_inventory_operation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail="Failed to validate inventory item"
        )
...
class UploadParams:
    def __init__(
        self,
        batch_size: Annotated[int, Query(ge=1, le=10000, description="Batch processing size")] = 1000,
        validate_only: Annotated[bool, Query(description="Only validate, don't import")] = False,
        overwrite_existing: Annotated[bool, Query(description="Overwrite existing records")] = False
    ):
...
class ErrorContext:
    ...
    def create_error_response(self, error: Exception, status_code: int = 500) -> HTTPException:
        ...
        return HTTPException(
            ...
            detail=f"Failed to {self.operation} {self.resource_type}"
        )
