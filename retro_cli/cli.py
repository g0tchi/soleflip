...
from utils import show_banner, keygen_animation, progress_bar, colored_text, clear_screen, wait_for_key, status_icon, format_status_line, section_header
...
class RetroAdminCLI:
    ...
    def show_system_status(self):
        ...
        print(format_status_line("Database", "ok" if db_status else "error", 
                                "PostgreSQL connection active" if db_status else "Connection failed"))
        print(format_status_line("Shopify API", "ok" if shopify_status else "missing",
                                "REST API connected" if shopify_status else "Check credentials"))
        print(format_status_line("StockX Integration", "ok" if stockx_status else "error",
                                "Service available" if stockx_status else "Service unavailable"))
        print(format_status_line("Awin Import", "ok" if awin_status else "missing",
    ...
    def analyze_products_for_enrichment(self):
        ...
        clear_screen()
        print(status_icon("processing") + colored_text(" ANALYZING PRODUCTS FOR ENRICHMENT", "purple"))
        print(colored_text("=" * 60, "purple"))
                ...
                async with db_manager.get_session() as session:
                    ...
                    if products:
                        ...
                        for i, product in enumerate(products, 1):
                            ...
                            missing = []
                            if not desc: missing.append("desc")
                            if not retail: missing.append("price") 
                            if not release: missing.append("release")
                        ...
                        for i, product in enumerate(sample_products, 1):
                            ...
                            status = []
                            if desc: status.append("✓ desc")
                            else: status.append("✗ desc")
                            if retail: status.append("✓ price") 
                            else: status.append("✗ price")
                            if release: status.append("✓ release")
                            else: status.append("✗ release")
    ...
    def search_and_match_stockx(self):
        ...
        clear_screen()
        print(status_icon("processing") + colored_text(" SEARCH & MATCH STOCKX PRODUCTS", "bright_green"))
                ...
                async with db_manager.get_session() as session:
                    ...
                    if results and 'results' in results:
                        ...
                        for i, product in enumerate(products[:10], 1):
                            ...
                            print(colored_text(f"    Retail: ${retail_price} | Release: {release_date}", "dim"))
                    else:
            ...
            async def enrich_products():
                ...
                async with db_manager.get_session() as session:
                    ...
                    for product in selected_products:
                        ...
                        search_results = await stockx_service.search_stockx_catalog(name, page_size=5)
                        
                        if search_results and 'results' in search_results and search_results['results']:
                            stockx_product = search_results['results'][0]  # Take best match
                            
                            # Extract enrichment data
                            new_description = stockx_product.get('description') or stockx_product.get('title')
                            ...
                            if not release_date and new_release_date:
                                try:
                                    ...
                                    update_data['release_date'] = datetime.fromisoformat(new_release_date.replace('Z', '+00:00'))
                                except:
                                    pass
                            ...
                            if update_data:
                                ...
                                enriched_count += 1
                                print(colored_text(f"  ✓ Updated: {', '.join(update_data.keys())}", "green"))
                            else:
                    ...
                    print(colored_text(f"\nEnrichment complete! Updated {enriched_count} products.", "bright_green"))
            ...
            async def bulk_enrich():
                ...
                async with db_manager.get_session() as session:
                    ...
                    for i, product in enumerate(products, 1):
                        ...
                        try:
                            # Search StockX for this product
                            search_results = await stockx_service.search_stockx_catalog(name, page_size=3)
                            ...
                            if search_results and 'results' in search_results and search_results['results']:
                                ...
                                for result in search_results['results']:
                                    ...
                                    if name.lower() in result_title or any(word in result_title for word in name.lower().split()[:2]):
                                        best_match = result
                                        break
                                
                                if not best_match:
                                    best_match = search_results['results'][0]  # Fallback to first result
                                
                                # Extract enrichment data
                                new_description = best_match.get('description') or best_match.get('title')
                                ...
                                update_data = {}
                                if not description and new_description:
                                    update_data['description'] = new_description[:1000]  # Limit length
                                if not retail_price and new_retail_price:
                                    try:
                                        update_data['retail_price'] = float(new_retail_price)
            ...
            async def show_stats():
                ...
                async with db_manager.get_session() as session:
                    ...
                    if total == 0:
                        ...
                        print(colored_text(f"With Description: {stats[1]} ({desc_pct:.1f}%)", "white"))
                        print(colored_text(f"With Retail Price: {stats[2]} ({retail_pct:.1f}%)", "white"))
                        print(colored_text(f"With Release Date: {stats[3]} ({release_pct:.1f}%)", "white"))
                        print(colored_text(f"With Resale Price: {stats[4]} ({resale_pct:.1f}%)", "white"))
