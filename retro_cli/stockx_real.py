...
class RealStockXManager:
    ...
    def sync_portfolio(self):
        ...
        try:
            ...
            print(colored_text(f"  - Total Items: {hist_count + active_count + listing_count}", "bright_green"))
        ...
        try:
            ...
            async def get_export_data():
                ...
                async with db_manager.get_session() as session:
                    ...
                    return await service.get_historical_orders(from_date, to_date, order_status="COMPLETED")
            ...
            with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for order in completed_orders:
                    ...
                    writer.writerow([order_date[:10], product_name, variant_name, gross_amount, fees, net_amount])
            ...
            if completed_orders:
                total_gross = sum(order.get('grossAmount', {}).get('amount', 0) for order in completed_orders)
                total_fees = sum(order.get('fees', {}).get('amount', 0) for order in completed_orders)
                print(colored_text(f"  - Gross Revenue: ${total_gross:.2f}", "cyan"))
                print(colored_text(f"  - Total Fees: ${total_fees:.2f}", "yellow"))
                print(colored_text(f"  - Net Profit: ${total_gross - total_fees:.2f}", "bright_green"))
    ...
    def show_api_status(self):
        ...
        print(colored_text(f"  StockX Service: {'[OK] AVAILABLE' if service_available else '[NO] UNAVAILABLE'}", 
                          "green" if service_available else "red"))
        print(colored_text(f"  API Connection: {'[OK] CONNECTED' if api_connected else '[NO] ERROR'}", 
