...
async def validate_pricing_system():
    ...
    conn = await asyncpg.connect('postgresql://metabaseuser:metabasepass@192.168.2.45:2665/metabase')
    ...
    try:
        ...
        schemas = await conn.fetch("SELECT schema_name FROM information_schema.schemata WHERE schema_name IN ('pricing', 'analytics')")
        ...
        """, rule_id, "Test Cost-Plus Rule", "cost_plus", 
            '{"brand": "Nike"}', '{"markup": 1.25}', 1, True)
        ...
        await conn.execute("""
            ...
            VALUES ($1, $2, $3, $4, $5)
        """, brand_id, "Nike", Decimal("1.15"), "Premium brand markup", True)
        ...
        await conn.execute("""
            ...
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        """, price_id, product_id, "AIR-JORDAN-1-HIGH", Decimal("180.00"), Decimal("25.5"), Decimal("0.85"), "cost_plus")
        ...
        """, forecast_id, product_id, date.today(), "daily", 5, Decimal("900.00"), Decimal("0.78"), "ensemble")
        ...
        # Query pricing rules
        rules = await conn.fetch("SELECT name, rule_type, is_active FROM pricing.price_rules WHERE is_active = true")
        print(f"   [OK] Active pricing rules: {len(rules)}")
        for rule in rules:
            print(f"     - {rule['name']} ({rule['rule_type']})")
        
        # Query brand multipliers
        multipliers = await conn.fetch("SELECT brand, multiplier FROM pricing.brand_multipliers WHERE is_active = true")
        ...
        for forecast in forecasts:
            print(f"     - {forecast['forecast_date']}: {forecast['predicted_quantity']} units, EUR{forecast['predicted_revenue']}")
        ...
        # Average margin
        avg_margin = await conn.fetchval("SELECT ROUND(AVG(margin_percentage), 2) FROM pricing.price_history")
