...
from shared.database.models import (
    Brand, Category, Size, Product, InventoryItem, 
    Supplier, Platform, Transaction, ImportBatch, ImportRecord
)
from shared.types.domain_types import (
    ProductStatus, InventoryStatus, SizeRegion, 
    SupplierType, SupplierStatus, BusinessSize,
    TransactionStatus, PlatformType, ImportSourceType, ImportStatus
)
...
class ProductFactory(factory.Factory):
    ...
    retail_price = factory.LazyFunction(lambda: Decimal(str(factory.Faker('random_int', min=50, max=500).generate())))
    avg_resale_price = factory.LazyAttribute(lambda obj: obj.retail_price * Decimal('1.2') if obj.retail_price else None)
...
class SupplierFactory(factory.Factory):
    ...
    supplier_type = factory.Iterator([
        SupplierType.RETAILER, SupplierType.WHOLESALER, 
        SupplierType.MANUFACTURER, SupplierType.INDIVIDUAL
    ])
    business_size = factory.Iterator([
        BusinessSize.SMALL, BusinessSize.MEDIUM, BusinessSize.LARGE
    ])
    ...
    preferred = False
    verified = True
    rating = factory.LazyFunction(lambda: Decimal(str(factory.Faker('random_int', min=3, max=5).generate())))
    
    # Statistics
    total_orders_count = factory.Faker('random_int', min=0, max=100)
    total_order_value = factory.LazyFunction(lambda: Decimal(str(factory.Faker('random_int', min=1000, max=50000).generate())))
...
class InventoryItemFactory(factory.Factory):
    ...
    purchase_price = factory.LazyFunction(lambda: Decimal(str(factory.Faker('random_int', min=100, max=400).generate())))
    purchase_date = factory.Faker('date_time_this_year', tzinfo=timezone.utc)
    supplier = factory.Faker('company')
    status = factory.Iterator([
        InventoryStatus.IN_STOCK, InventoryStatus.LISTED_STOCKX, 
        InventoryStatus.SOLD, InventoryStatus.RESERVED
    ])
...
class TransactionFactory(factory.Factory):
    ...
    sale_price = factory.LazyFunction(lambda: Decimal(str(factory.Faker('random_int', min=200, max=800).generate())))
    platform_fee = factory.LazyAttribute(lambda obj: obj.sale_price * Decimal('0.085') if obj.sale_price else Decimal('0'))
    shipping_cost = factory.LazyFunction(lambda: Decimal(str(factory.Faker('random_int', min=10, max=50).generate())))
    net_profit = factory.LazyAttribute(lambda obj: obj.sale_price - obj.platform_fee - obj.shipping_cost)
    status = factory.Iterator([
...
class ImportBatchFactory(factory.Factory):
    ...
    source_type = factory.Iterator([
        ImportSourceType.CSV_UPLOAD, ImportSourceType.STOCKX_API,
        ImportSourceType.MANUAL_ENTRY
    ])
    source_file = factory.Faker('file_path', extension='csv')
    total_records = factory.Faker('random_int', min=10, max=1000)
    processed_records = factory.LazyAttribute(lambda obj: min(obj.total_records, factory.Faker('random_int', min=0, max=obj.total_records).generate()))
    ...
    completed_at = factory.LazyAttribute(lambda obj: obj.started_at + factory.Faker('time_delta', hours=1).generate() if obj.status == ImportStatus.COMPLETED else None)
    created_at = factory.LazyFunction(lambda: datetime.now(timezone.utc))
    updated_at = factory.LazyFunction(lambda: datetime.now(timezone.utc))
...
class ImportRecordFactory(factory.Factory):
    ...
    id = factory.LazyFunction(uuid.uuid4)
    source_data = factory.LazyFunction(lambda: {
    ...
    status = factory.Iterator([
        ImportStatus.PENDING, ImportStatus.COMPLETED, ImportStatus.FAILED
    ])
    processing_started_at = factory.Faker('date_time_this_year', tzinfo=timezone.utc)
    processing_completed_at = factory.LazyAttribute(lambda obj: obj.processing_started_at + factory.Faker('time_delta', minutes=1).generate() if obj.status == ImportStatus.COMPLETED else None)
    error_message = None
    created_at = factory.LazyFunction(lambda: datetime.now(timezone.utc))
    updated_at = factory.LazyFunction(lambda: datetime.now(timezone.utc))
