    ...
    async def run_load_test(
        ...
        for i in range(concurrent_users):
            ...
            task = asyncio.create_task(
                self._worker(requests, duration_seconds, delay, semaphore)
            )
            tasks.append(task)
        ...
        try:
            async with session.request(
                ...
                return LoadTestResult(
                    ...
                    success=success,
                    error=None if success else f"Expected {request.expected_status}, got {response.status}"
                )
    ...
    def _generate_report(self, total_time: float) -> Dict[str, Any]:
        ...
        for endpoint, data in endpoint_stats.items():
            if data["response_times"]:
                ...
                data["success_rate"] = round(data["successful_requests"] / data["total_requests"] * 100, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test-type", choices=["basic", "stress"], default="basic",
    ...
    async def main():
        ...
        for endpoint, stats in results['endpoint_performance'].items():
            print(f"  {endpoint}: {stats['avg_response_time_ms']:.2f}ms avg, {stats['success_rate']:.1f}% success")
