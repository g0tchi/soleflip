...
@pytest.mark.asyncio
async def test_get_stockx_product_details_success(async_client, api_helper, mock_external_services, override_get_db):
    """
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_get_stockx_product_details_not_found(async_client, api_helper, mock_external_services, override_get_db):
    """
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_search_stockx_products_success(async_client, api_helper, mock_external_services, override_get_db):
    """
    ...
    # Assertions
    assert response_data == mock_response_data
    mock_external_services['stockx'].search_stockx_catalog.assert_called_once_with(query=search_query, page=1, page_size=5)
