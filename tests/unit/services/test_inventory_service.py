...
async def test_sync_inventory_from_stockx_update_existing(inventory_service, mock_stockx_service):
    ...
    # 1. Mock existing product and its inventory
    existing_item = InventoryItem(id=uuid4(), external_ids={"stockx": "variant-existing"}, quantity=5)
    product = Product(
        id=product_id,
        sku="TEST-SKU",
        category_id=category_id,
...
async def test_get_inventory_overview(inventory_service):
    ...
    """
    # Arrange
    inventory_service.inventory_repo.get_all = AsyncMock(return_value=[
...
async def test_create_product_with_inventory(inventory_service):
    ...
    inventory_service.category_repo.find_one_or_create = AsyncMock(return_value=Category(id=uuid4()))
    inventory_service.product_repo.find_by_sku = AsyncMock(return_value=None)
    inventory_service.size_repo.find_one_or_create = AsyncMock(return_value=Size(id=uuid4()))
    inventory_service.product_repo.create_with_inventory = AsyncMock(return_value=Product(id=uuid4(), sku="TEST-SKU"))

    # Act
    product = await inventory_service.create_product_with_inventory(product_request, inventory_requests)
...
async def test_update_inventory_status(inventory_service):
    ...
    inventory_service.product_repo.update_inventory_status.assert_called_once_with(inventory_id, new_status, None)
...
async def test_get_low_stock_alert(inventory_service):
    ...
    """
    # Arrange
    inventory_service.product_repo.get_low_stock_products = AsyncMock(return_value=[{"name": "Test Product", "stock": 4}])
