...
class SecurityManager:
    ...
    def update_session_activity(self, action: str, details: Optional[Dict] = None):
        ...
        if details:
            self.log_audit("USER_ACTION", f"User {self.current_session['username']} performed {action}: {details}")
    ...
    def end_session(self):
        ...
        self.log_audit("USER_LOGOUT", f"User {username} ended session {session_id} after {duration}")
    ...
    def validate_environment(self) -> bool:
        ...
        if self.config.system.is_test:
            self.log_audit("ENV_CHECK", "Running in TEST mode - database operations will use test schema")
            return True
...
def display_security_status(security_manager: SecurityManager):
    ...
    if session_stats["current_session"]:
        print(colored_text(f"Session: ACTIVE ({session_stats['session_duration']})", "bright_green"))
