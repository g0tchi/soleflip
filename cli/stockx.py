...
class StockXManager:
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                result = session.execute(
                    text("SELECT * FROM api_key WHERE name ILIKE '%stock%' LIMIT 1")
                )
    ...
    def list_portfolio_items(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                result = session.execute(
                    text(
                        "SELECT name, display_name FROM metabase_table WHERE name ILIKE '%stock%' LIMIT 10"
                    )
                )
                ...
                if stockx_tables:
                    ...
                    try:
                        ...
                        print(
                            colored_text(
                                "\\nNote: Showing demo data (direct table access requires configuration)",
                                "yellow",
                            )
                        )
                    except Exception as e:
            ...
            for i, item in enumerate(portfolio_items, 1):
                ...
                print(
                    colored_text(f"    Colorway: {item['colorway']} | Size: {item['size']}", "dim")
                )
                print(
                    colored_text(
                        f"    Purchase: ${item['purchase_price']} | Current: ${item['current_value']}",
                        "white",
                    )
                )
                print(
                    colored_text(
                        f"    Profit: ${profit:+.0f} | Status: {item['status']}", profit_color
                    )
                )
                print()
    ...
    def search_products(self):
        ...
        try:
            ...
            for i, product in enumerate(results, 1):
                ...
                print(
                    colored_text(
                        f"   Brand: {product['brand']} | Volatility: {product['volatility']}", "dim"
                    )
                )
                print(
                    colored_text(
                        f"   Retail: ${product['retail_price']} | Market: ${product['market_price']}",
                        "white",
                    )
                )
    ...
    def show_api_status(self):
        ...
        print(
            colored_text(
                f"  Database: {'[OK] CONNECTED' if db_connected else '[NO] OFFLINE'}",
                "green" if db_connected else "red",
            )
        )
        print(
            colored_text(
                f"  API Config: {'[OK] CONFIGURED' if api_configured else '[NO] MISSING'}",
                "green" if api_configured else "red",
            )
        )
        print(
            colored_text(
                f"  API Access: {'[OK] CONNECTED' if api_connected else '[NO] UNAVAILABLE'}",
        ...
        if api_configured:
            ...
            print(
                colored_text(
                    f"  Token: {'***configured***' if self.stockx_config['api_token'] else 'missing'}",
                    "cyan",
                )
            )
        ...
        if db_connected:
            try:
                with db.get_session() as session:
                    ...
                    result = session.execute(
                        text(
                            "SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name ILIKE '%stock%'"
                        )
                    )
