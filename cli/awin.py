...
class AwinManager:
    ...
    def check_status(self) -> bool:
        ...
        try:
            ...
            return response.status_code in [200, 401]  # 401 means API is working but token might be invalid
    ...
    def _add_import_record(self, import_type: str, filename: str, record_count: int, status: str = "success"):
    ...
    def _validate_csv_data(self, csv_file: str, delimiter: str, total_rows: int):
        ...
        try:
                    ...
                    for field in numeric_fields:
                        if field in row and row[field].strip():
                            try:
                                ...
                                validation_results["errors"].append(f"Row {row_num}: Invalid {field} '{row[field]}'")
                                is_valid = False
            ...
            print(colored_text(f"Errors: {len(validation_results['errors'])}", "red" if validation_results['errors'] else "green"))
            print(colored_text(f"Warnings: {len(validation_results['warnings'])}", "yellow" if validation_results['warnings'] else "green"))
            ...
            # Record validation
            self._add_import_record("validation", csv_file, validation_results['total_rows'], 
    ...
    def _export_csv_to_json(self, csv_file: str, delimiter: str, total_rows: int):
        ...
        output_file = csv_file.replace('.csv', f'_processed_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
        ...
        try:
            ...
            print(colored_text(f"✅ Exported {data['metadata']['total_records']:,} records to:", "bright_green"))
            print(colored_text(f"   {output_file}", "cyan"))
            
            self._add_import_record("json_export", csv_file, data['metadata']['total_records'], "success")
    ...
    def import_history(self):
        ...
        # Show recent imports
        recent_imports = sorted(self.import_history, key=lambda x: x['timestamp'], reverse=True)[:20]
        ...
        for i, record in enumerate(recent_imports, 1):
            ...
            print(colored_text(f"{i:2d}. {timestamp} | {import_type:12} | {filename:30} | {count:6,} records", "white"))
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    print(colored_text(f"Account ID: {account_data.get('account_id', 'Unknown')}", "cyan"))
                except:
