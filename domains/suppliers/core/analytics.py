...
async def get_supplier_category_distribution(db: AsyncSession) -> dict:
    ...
    query = select(Supplier.supplier_category, func.count(Supplier.id)).group_by(
...
async def get_total_supplier_revenue(db: AsyncSession, supplier_id: str) -> float:
    ...
    query = select(func.sum(SupplierAccount.total_spent)).where(
...
async def get_supplier_intelligence_dashboard(db: AsyncSession) -> Dict:
    ...
    return {"summary": {"total_suppliers": total_suppliers}, "category_distribution": category_dist}
