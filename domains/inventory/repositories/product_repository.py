...
class ProductRepository(BaseRepository[Product]):
    ...
    async def search(
        ...
        if brand_filter:
            brand_condition = (
                select(Brand.id)
                .where(Brand.name.ilike(f"%{brand_filter}%"))
            )
        ...
        if category_filter:
            category_condition = (
                select(Category.id)
                .where(Category.path.op('<@')(category_filter))
            )
        ...
        return [
            {
                ...
                'sold': row.sold or 0,
                'avg_purchase_price': float(row.avg_purchase_price) if row.avg_purchase_price else None
            }
            for row in rows
        ]
