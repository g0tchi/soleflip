...
class PredictiveInsightsService:
            ...
            try:
                ...
                if recommendation and recommendation.roi_estimate >= min_roi:
                    if investment_budget:
                        if total_investment + recommendation.investment_required <= investment_budget:
    ...
    async def _generate_restock_insights(
        ...
        for product in products:
            try:
                ...
                predicted_demand = sum(p['forecasted_units'] for p in forecast.predictions[:days_ahead])
                ...
                if predicted_demand > current_stock * 1.5:  # Demand exceeds 1.5x current stock
                    ...
                    insight = PredictiveInsight(
                        ...
                        priority=self._calculate_priority(predicted_demand - current_stock, confidence),
                        ...
                        supporting_data={
                            ...
                            "market_trend": market_data.get('trend', 'stable') if market_data else 'unknown',
                            "days_until_stockout": max(1, int(current_stock / (predicted_demand / days_ahead))),
                        },
                ...
                self.logger.error(f"Failed to generate restock insight for product {product.id}: {str(e)}")
                continue
    ...
    async def _generate_demand_surge_insights(
        ...
        for product in products:
            try:
                ...
                # Calculate trend
                recent_predictions = [p['forecasted_units'] for p in forecast.predictions[:14]]  # Next 2 weeks
                ...
                if surge_factor > 1.5:  # 50% increase
                    ...
                    insight = PredictiveInsight(
                        ...
                        priority=InsightPriority.HIGH if surge_factor > 2.0 else InsightPriority.MEDIUM,
                        ...
                        supporting_data={
                            ...
                            "market_conditions": market_data.get('market_conditions', 'unknown') if market_data else 'unknown',
                ...
                self.logger.error(f"Failed to generate surge insight for product {product.id}: {str(e)}")
                continue
        ...
        for product in products:
            try:
                ...
                if seasonal_multiplier > 1.1:
                    forecast = await self._get_product_demand_forecast(product, days_ahead)
                    base_demand = sum(p['forecasted_units'] for p in forecast.predictions) if forecast else 30
                    ...
                    insight = PredictiveInsight(
                        ...
                        supporting_data={
                            ...
                            "peak_months": [m for season in active_seasons for m in seasonal_patterns[season]],
                        },
                ...
                self.logger.error(f"Failed to generate seasonal insight for product {product.id}: {str(e)}")
                continue
    ...
    async def _generate_market_shift_insights(
        ...
        for product in products:
            try:
                ...
                self.logger.error(f"Failed to generate market shift insight for product {product.id}: {str(e)}")
                continue
        ...
        for product in products:
            try:
                ...
                if profit_increase > 0.1:  # 10% profit increase potential
                    forecast = await self._get_product_demand_forecast(product, days_ahead)
                    predicted_sales = sum(p['forecasted_units'] for p in forecast.predictions) if forecast else 20
                    
                    potential_profit = Decimal(str(profit_increase)) * current_price * Decimal(str(predicted_sales))
                    
                    insight = PredictiveInsight(
                        insight_id=f"profit_{product.id}_{int(datetime.now().timestamp())}",
                        insight_type=InsightType.PROFIT_OPTIMIZATION,
                        priority=InsightPriority.HIGH if profit_increase > 0.2 else InsightPriority.MEDIUM,
                        ...
                        supporting_data={
                            ...
                            "pricing_strategy": pricing_analysis.get('recommended_strategy', 'dynamic'),
                        },
                ...
                self.logger.error(f"Failed to generate profit insight for product {product.id}: {str(e)}")
                continue
            ...
            try:
                ...
                if days_to_clear > 120:  # More than 4 months to clear
                    ...
                    insight = PredictiveInsight(
                        ...
                        priority=InsightPriority.MEDIUM if days_to_clear > 180 else InsightPriority.LOW,
                ...
                self.logger.error(f"Failed to generate clearance insight for product {product.id}: {str(e)}")
                continue
    ...
    async def _generate_product_forecast(
        ...
        try:
            ...
            return InventoryForecast(
                ...
                confidence_level=forecast_result.model_metrics.get('r2_score', 0.7) if forecast_result.model_metrics else 0.7,
                seasonal_factors={
    ...
    async def _create_restock_recommendation(
        ...
        try:
            ...
            projected_revenue = estimated_selling_price * Decimal(str(forecast.predicted_demand_30d))
            projected_profit = projected_revenue - (estimated_cost_per_unit * Decimal(str(forecast.predicted_demand_30d)))
            
            # Calculate ROI
            roi_estimate = float(projected_profit / investment_required) if investment_required > 0 else 0
