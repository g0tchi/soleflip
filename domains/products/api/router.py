...
async def enrich_product_data(
    background_tasks: BackgroundTasks,
    product_ids: Optional[List[str]] = Query(None, description="Specific product IDs to enrich (if none provided, enriches all products needing enrichment)")
):
            ...
            try:
                ...
                for product in products:
                    try:
                        ...
                        search_query = f"{product.brand_name} {product.name}" if product.brand_name else product.name
                        search_results = await stockx_service.search_stockx_catalog(search_query, page=1, page_size=5)
                        ...
                        if search_results and search_results.get('products'):
                            ...
                            await bg_session.execute(update_query, {
                                "product_id": product.id,
                                "sku": stockx_product.get('id'),
                                "description": stockx_product.get('description', '')[:500],  # Limit description length
                                "retail_price": stockx_product.get('retailPrice'),
                                "release_date": stockx_product.get('releaseDate')
                            })
                            
                            enriched_count += 1
                            logger.info("Enriched product", product_id=product.id, sku=stockx_product.get('id'))
                            
                    except Exception as e:
                        logger.warning("Failed to enrich product", product_id=product.id, error=str(e))
                        continue
                
                await bg_session.commit()
                logger.info("Product enrichment completed", enriched_count=enriched_count, total_processed=len(products))
                
            except Exception as e:
                logger.error("Product enrichment background task failed", error=str(e), exc_info=True)
...
async def get_enrichment_status(
    ...
    try:
        ...
        return {
            ...
            "enrichment_stats": {
                ...
                "description": {
                    ...
                    "completion_percentage": round((stats.products_with_description / total) * 100, 1)
                },
                "retail_price": {
                    "completed": stats.products_with_retail_price,
                    "missing": stats.products_needing_retail_price,
                    "completion_percentage": round((stats.products_with_retail_price / total) * 100, 1)
                },
                "release_date": {
                    "completed": stats.products_with_release_date,
                    "missing": stats.products_needing_release_date,
                    "completion_percentage": round((stats.products_with_release_date / total) * 100, 1)
                }
            },
            "overall_completion": round(((stats.products_with_sku + stats.products_with_description + 
                                       stats.products_with_retail_price + stats.products_with_release_date) / (total * 4)) * 100, 1)
        }
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to fetch enrichment status"
        )
...
async def get_product_stats(
    ...
    try:
        ...
        if sales_data and sales_data.total_revenue and sales_data.total_profit:
            avg_profit_margin = (float(sales_data.total_profit) / float(sales_data.total_revenue)) * 100
