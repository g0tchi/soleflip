...
class ProductProcessor:
        ...
        for name, data in product_data.items():
            ...
            candidates.append(
                ProductCandidate(
                    ...
                    avg_price=(
                        float(sum(data["prices"]) / len(data["prices"])) if data["prices"] else 0.0
                    ),
    ...
    async def create_products_from_candidates(
        ...
        for candidate in candidates:
            try:
                ...
                stats["errors"] += 1
                logger.error(
                    "Product creation from candidate failed", name=candidate.name, error=str(e)
                )
