...
class ProductProcessor:
    ...
    async def enrich_product_from_stockx(self, product_id: UUID) -> Optional[Product]:
        """
        Enriches a product with additional data from the StockX Catalog API.
        """
        logger.info("Enriching product from StockX", product_id=str(product_id))

        product = await self.db_session.get(Product, product_id)
        if not product:
            logger.warning("Product not found for enrichment", product_id=str(product_id))
            return None

        stockx_service = StockXService(self.db_session)

        # Assumption: The product's SKU can be used as the StockX Product ID.
        # This might need adjustment if the SKU is not the correct identifier.
        if not product.sku:
            logger.warning("Product has no SKU, cannot enrich from StockX", product_id=str(product_id))
            return product

        stockx_data = await stockx_service.get_product_details(product.sku)

        if not stockx_data:
            logger.info("No data returned from StockX Catalog API for product", product_id=str(product_id), sku=product.sku)
            return product

        # Update product fields with data from StockX
        # We use .get() to avoid errors if a key is missing in the response
        product.name = stockx_data.get('title', product.name) # StockX title is often better

        # Potentially update other fields if they exist in our model
        # For example, if we add a 'style_id' field to our Product model:
        # if 'styleId' in stockx_data:
        #     product.style_id = stockx_data['styleId']

        # If the brand from StockX is different, we could update it,
        # but this might require careful handling (e.g., creating the new brand).
        # For now, we only update the name.

        await self.db_session.commit()
        logger.info("Successfully enriched product with StockX data", product_id=str(product_id))
