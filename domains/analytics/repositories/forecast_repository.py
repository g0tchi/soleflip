...
from domains.pricing.models import SalesForecast, ForecastAccuracy, DemandPattern, PricingKPI
from shared.database.models import Product, Brand, Category, Platform, Transaction, InventoryItem
...
class ForecastRepository:
    ...
    async def get_latest_forecasts(
        ...
        query = select(SalesForecast).where(
            ...
            SalesForecast.forecast_date.asc()
        )
    ...
    async def get_forecast_by_run(
        ...
        query = select(SalesForecast).where(
            SalesForecast.forecast_run_id == forecast_run_id
        ).options(
            joinedload(SalesForecast.product),
            joinedload(SalesForecast.brand)
        )
    ...
    async def get_model_accuracy_history(
        ...
        query = select(ForecastAccuracy).where(
    ...
    async def calculate_forecast_accuracy(
        ...
        actual_values = [actual_lookup[(entity_id, fc.forecast_date)]['units_sold'] 
                        for fc in forecasts 
                        if (str(fc.product_id or fc.brand_id or fc.category_id), fc.forecast_date) in actual_lookup]
    ...
    async def create_demand_pattern(
        self,
    ...
    async def get_demand_patterns(
        ...
        query = select(DemandPattern).where(
    ...
    async def get_pricing_kpis(
        ...
        query = select(PricingKPI).where(
            PricingKPI.aggregation_level == aggregation_level,
            PricingKPI.kpi_date >= start_date
