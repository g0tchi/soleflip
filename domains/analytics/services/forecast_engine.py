...
from domains.pricing.models import SalesForecast, ForecastAccuracy
...
class ForecastEngine:
    ...
    async def _forecast_single_entity(
        ...
        # Format predictions
        prediction_list = self._format_predictions(
            predictions, confidence_intervals, config
        )
    ...
    async def _linear_trend_model(
        ...
        df: pd.DataFrame,
        config: ForecastConfig
    ) -> Tuple[List[float], List[Tuple[float, float]], Dict[str, float], Optional[Dict[str, float]]]:
    ...
    async def _random_forest_model(
        ...
        feature_cols = [col for col in df.columns if col not in ['period_date', 'units_sold', 'entity_id']]
    ...
    def _create_ml_features(
        ...
        for window in [3, 7, 14]:
            if window < len(df):
                df[f'units_sold_ma_{window}'] = df['units_sold'].rolling(window, min_periods=1).mean()
    ...
    async def _get_forecastable_entities(
        self,
        level: ForecastLevel
    ) -> List[uuid.UUID]:
