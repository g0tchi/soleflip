...
class PricingEngine:
    ...
    async def calculate_optimal_price(
        ...
        strategies: Optional[List[PricingStrategy]] = None
    ) -> PricingResult:
    ...
    async def _calculate_cost_plus_price(
        ...
        # Calculate margins
        margin_percent = ((suggested_price - base_cost) / suggested_price * Decimal('100')).quantize(
    ...
    async def _calculate_market_based_price(
        ...
        if market_rules:
            ...
            if rule.seasonal_adjustments:
                ...
                reasoning.append(f"Seasonal adjustment for month {datetime.now().month}: {seasonal_adj}")
    ...
    async def _calculate_dynamic_price(
        ...
        total_weight = sum(
            weights.get(strategy, Decimal('0.3'))
            for strategy in strategy_prices.keys()
        )
    ...
    def _select_best_pricing_result(
        self,
        results: Dict[PricingStrategy, PricingResult],
        context: PricingContext
    ) -> PricingResult:
    ...
    async def _apply_final_adjustments(
        ...
        for rule in rules:
            if rule.minimum_margin_percent:
                base_cost = await self._estimate_product_cost(context.product)
                min_price = base_cost / (Decimal('1') - rule.minimum_margin_percent / Decimal('100'))
    ...
    async def _estimate_product_cost(self, product: Product) -> Decimal:
        ...
        recent_purchase = await self.repository.get_latest_price(
            product.id, price_type='purchase'
        )
        ...
        if market_prices:
            avg_market = sum(mp.last_sale or mp.average_price or Decimal('0') 
                           for mp in market_prices) / len(market_prices)
