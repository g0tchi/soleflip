...
class AutoListingService:
    ...
    async def execute_listing_automation(
        self, max_items: int = 100, dry_run: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for item in items:
                await self.db_session.refresh(
                    item, ["product", "product.brand", "product.category"]
                )
    ...
    async def _evaluate_rule_conditions(
        self, item: InventoryItem, rule: ListingRule, market_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for condition, value in rule.conditions.items():
            ...
            # Profit margin condition
                ...
                if item.purchase_price and market_context.get("market_price"):
                    margin = (
                        (market_context["market_price"] - item.purchase_price) / item.purchase_price
                    ) * 100
            ...
            # Purchase age condition
                ...
                if item.purchase_date:
                    age_days = (
                        datetime.now(timezone.utc) - item.purchase_date.replace(tzinfo=timezone.utc)
                    ).days
    ...
    async def _execute_listing(
        self, item: InventoryItem, listing_decision: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _calculate_listing_price(
        self, item: InventoryItem, strategy: str, markup_percent: float
    ) -> Decimal:
    ...
    async def _create_stockx_listing(
        self, item: InventoryItem, price: Decimal, expires_in_days: int
    ) -> Dict[str, Any]:
    ...
    async def _update_item_status_after_listing(self, item: InventoryItem, new_status: str) -> bool:
    ...
    async def simulate_rule_execution(
        self, rule_name: Optional[str] = None, max_items: int = 10
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.execute_listing_automation(max_items=max_items, dry_run=True)

            return {"simulation_complete": True, "rule_tested": rule_name, **result}
