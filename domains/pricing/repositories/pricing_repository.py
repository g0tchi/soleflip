...
class PricingRepository:
    ...
    async def get_active_price_rules(
        ...
        query = select(PriceRule).where(
            ...
            or_(PriceRule.effective_until.is_(None), PriceRule.effective_until >= datetime.now())
        ).options(
        ...
        if category_id:
            query = query.where(or_(PriceRule.category_id == category_id, PriceRule.category_id.is_(None)))
        if platform_id:
            query = query.where(or_(PriceRule.platform_id == platform_id, PriceRule.platform_id.is_(None)))
        if rule_type:
    ...
    async def update_price_rule(self, rule_id: uuid.UUID, update_data: Dict[str, Any]) -> Optional[PriceRule]:
    ...
    async def get_brand_multipliers(
        ...
        query = select(BrandMultiplier).where(
            BrandMultiplier.brand_id == brand_id,
            BrandMultiplier.active == True,
            BrandMultiplier.effective_from <= effective_date,
            or_(BrandMultiplier.effective_until.is_(None), BrandMultiplier.effective_until >= effective_date)
    ...
    async def get_price_history(
        ...
        query = select(PriceHistory).where(
            ...
            joinedload(PriceHistory.platform)
    ...
    async def get_latest_price(
        ...
        query = query.order_by(desc(PriceHistory.price_date), desc(PriceHistory.created_at)).limit(1)
    ...
    async def get_market_prices(
        ...
        ).options(joinedload(MarketPrice.product)).order_by(desc(MarketPrice.price_date))
    ...
    async def get_top_performing_products(
        ...
        if metric == 'volume':
            ...
            query = select(
                ...
                Product, MarketPrice.product_id == Product.id
            ).join(
                Brand, Product.brand_id == Brand.id
            ).where(
                MarketPrice.price_date >= start_date,
                MarketPrice.sales_volume.is_not(None)
            ).group_by(
                Product.id, Product.name, Product.sku, Brand.name
            ).order_by(
                desc('total_volume')
            ).limit(limit)
            ...
            subquery = select(
                ...
                func.first_value(PriceHistory.price_amount).over(
            ...
            query = select(
                ...
                ((subquery.c.latest_price - subquery.c.first_price) / subquery.c.first_price * 100).label('price_growth_percent')
            ).select_from(
                subquery
            ).join(
                Product, subquery.c.product_id == Product.id
            ).join(
                Brand, Product.brand_id == Brand.id
            ).where(
                subquery.c.first_price > 0
            ).order_by(
                desc('price_growth_percent')
            ).limit(limit)
        ...
        for row in rows:
            ...
            if metric == 'volume':
                ...
                product_dict.update({
                    ...
                    'growth_percent': float(row.price_growth_percent) if row.price_growth_percent else 0
                })
