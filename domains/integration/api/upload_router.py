    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error("Import processing failed", batch_id=str(batch.id), error=str(e), exc_info=True)
                return ImportResponse(
                    status="error",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to initiate StockX import: {str(e)}"
        )
...
async def get_import_status(
    ...
    try:
        ...
        if not batch:
            raise HTTPException(
                status_code=404,
                detail=f"Import batch with ID {batch_id} not found"
            )
