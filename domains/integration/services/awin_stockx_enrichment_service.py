...
class AwinStockXEnrichmentService:
    ...
    async def enrich_all_products(
        ...
        try:
            ...
            for i, product in enumerate(products, 1):
                try:
                    ...
                    if search_results and search_results.get('products'):
                        ...
                        stats["matched"] += 1
                        logger.debug(f"Matched product {i}/{len(products)}",
                                   ean=product.ean, stockx_id=stockx_product_id)
                    else:
                        # No match found
                    ...
                    logger.error("Product enrichment failed",
                               product_id=product.awin_product_id, error=str(e))
            ...
            stats["match_rate_percentage"] = round(
                (stats["matched"] / stats["total_processed"] * 100), 2
    ...
    async def _store_stockx_price_source(
        ...
        result = await self.session.execute(product_query, {"awin_product_id": str(awin_product_id)})
        row = result.fetchone()
        ...
        source_url = f"https://stockx.com/{stockx_data.get('urlKey')}" if stockx_data.get('urlKey') else None
        ...
        logger.debug("Stored StockX price in price_sources",
                    product_id=str(product_id),
    ...
    async def get_enrichment_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "match_rate_percentage": round(
                (stats.matched / stats.total_products * 100), 2
