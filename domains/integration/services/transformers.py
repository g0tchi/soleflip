...
class DataTransformer:
    ...
    def transform(self, data: List[Dict[str, Any]], 
                  ...
                  source_type: str = "unknown") -> TransformResult:
        """
        ...
        logger.info("Starting data transformation", 
                   records=len(data), mappings=len(field_mappings), source=source_type)
        ...
        for mapping in field_mappings:
            try:
                ...
                if value is not None:
                    ...
                    record_errors.append(f"Required field '{mapping.source_field}' is missing or empty")
    ...
    def _extract_field_value(self, record: Dict[str, Any], 
                           mapping: FieldMapping, row_idx: int) -> Any:
...
class NotionTransformer(DataTransformer):
    ...
    def _transform_notion_record(self, record: Dict[str, Any], row_idx: int) -> Optional[Dict[str, Any]]:
        ...
        for prop_name, prop_data in properties.items():
            ...
            if prop_type == 'title':
                ...
                if title_data:
                    transformed[prop_name.lower().replace(' ', '_')] = title_data[0].get('text', {}).get('content', '')
                    
            elif prop_type == 'rich_text':
                text_data = prop_data.get('rich_text', [])
                if text_data:
                    transformed[prop_name.lower().replace(' ', '_')] = text_data[0].get('text', {}).get('content', '')
                ...
                if date_data and date_data.get('start'):
                    transformed[prop_name.lower().replace(' ', '_')] = self._parse_date(date_data['start'])
...
def transform_data(data: List[Dict[str, Any]], 
                  source_type: str,
                  custom_mappings: Optional[List[FieldMapping]] = None) -> TransformResult:
    """
