...
class LargeRetailerImportService:
    ...
    async def import_json_file(
        self, file_path: str, retailer_name: str, import_name: Optional[str] = None
    ) -> UUID:
        """
    ...
    async def _create_import_batch(
        self, batch_id: UUID, source_type: str, filename: Optional[str], import_name: str
    ) -> ImportBatch:
        """Create import batch record"""
    ...
    async def _update_batch_status(
        self, batch_id: UUID, status: str, error_message: Optional[str] = None
    ):
        """Update batch status"""
