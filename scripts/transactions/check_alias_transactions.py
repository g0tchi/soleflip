...
async def check_alias_transactions():
    ...
    try:
        async with db.get_session() as session:
            ...
            latest_batch_result = await session.execute(text("""
            ...
            transactions_result = await session.execute(text("""
                SELECT 
            ...
            print(f"  Date range: {transaction_stats.earliest_date} to {transaction_stats.latest_date}")
            ...
            for tx in sample_transactions:
                print(f"  {tx.external_id}: {tx.product_name} (Size {tx.size}) - ${tx.sale_price} on {tx.platform_name}")
            ...
            error_result = await session.execute(text("""
                SELECT 
