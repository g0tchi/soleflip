...
async def create_test_inventory_items(session: AsyncSession, test_data: List[Dict[str, Any]]):
    ...
    for item_data in test_data:
        ...
        # Get the product ID
        product_id_result = await session.execute(
            text(
                """
        ...
        if inventory_row:
            ...
            print(
                f"[OK] Created inventory item {inventory_id} for {item_data['sku']} - {item_data['product_name']}"
            )
            created_items.append(
                {"inventory_id": inventory_id, "sku": item_data["sku"], "expected_data": item_data}
            )
        else:
...
async def calculate_bi_metrics_manually(
    session: AsyncSession, inventory_id: str, expected_data: Dict[str, Any]
):
    ...
    roi_percentage = (
        (profit / expected_data["net_buy"]) * 100 if expected_data["net_buy"] > 0 else 0
    )
    ...
    print(
        f"   Shelf Life: {shelf_life_days} days (expected: {expected_data['shelf_life_expected']})"
    )
...
async def validate_bi_calculations(session: AsyncSession, created_items: List[Dict[str, Any]]):
    ...
    for item in created_items:
        ...
        validation_result = await calculate_bi_metrics_manually(
            session, inventory_id, expected_data
        )
        ...
        roi_accuracy = (
            abs(calculated["roi_percentage"] - expected["roi_percentage"]) <= 5.0
        )  # 5% tolerance
        pas_accuracy = (
            abs(calculated["profit_per_shelf_day"] - expected["profit_per_shelf_day"]) <= 0.1
        )
        ...
        print(f"\n{sku}: {status}")
        print(
            f"  Shelf Life: {'[OK]' if shelf_life_accuracy else '[ERR]'} {calculated['shelf_life_days']} vs {expected['shelf_life_days']} days"
        )
        print(
            f"  ROI:        {'[OK]' if roi_accuracy else '[ERR]'} {calculated['roi_percentage']:.1f}% vs {expected['roi_percentage']:.1f}%"
        )
        print(
            f"  PAS:        {'[OK]' if pas_accuracy else '[ERR]'} {calculated['profit_per_shelf_day']:.2f} vs {expected['profit_per_shelf_day']:.2f} EUR"
        )
