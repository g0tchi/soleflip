...
async def create_brand_relationships():
    ...
    for parent_name, subsidiaries in parent_companies:
        try:
            ...
            parent_id = await conn.fetchval(
                "SELECT id FROM core.brands WHERE name = $1", parent_name
            )
            if not parent_id:
                parent_id = await conn.fetchval(
                    """
    ...
    famous_collabs = [
        (
            ...
            "limited_edition",
            date(2017, 9, 9),
            "Revolutionary deconstructed sneaker collection by Virgil Abloh",
            "legendary",
            2000000000,
            50000,
            8.5,
        ...
        (
            ...
            "ongoing",
            date(2017, 5, 13),
            "Hip-hop artist collaboration with Jordan Brand",
            "high",
            500000000,
            None,
            6.0,
        ...
        (
            ...
            "ongoing",
            date(2015, 6, 27),
            "Revolutionary lifestyle and performance collaboration",
            "legendary",
            3000000000,
            None,
            10.0,
        ...
        (
            ...
            "ongoing",
            date(2014, 3, 15),
            "High-fashion streetwear collaboration",
            "high",
            100000000,
            None,
            4.0,
    ...
    for (
        ...
        try:
            primary_id = await conn.fetchval(
                "SELECT id FROM core.brands WHERE name = $1", primary_brand
            )
            collab_id = await conn.fetchval(
                "SELECT id FROM core.brands WHERE name = $1", collaborator
            )
