...
async def show_brand_deep_dive_summary():
  ...
  {brand['name']} ({brand['company_size']})
    ...
    for innovation in innovations:
        print(
            f"  {innovation['innovation_year']}: {innovation['brand_name']} - {innovation['event_title']}"
        )
    ...
    for collab in collabs:
        revenue_str = (
            f"${collab['estimated_revenue_usd']:,}"
            if collab["estimated_revenue_usd"]
            else "Revenue N/A"
        )
        print(
            f"  {collab['primary_brand']} x {collab['collaborator_brand']} ({collab['success_level']})"
        )
        print(
            f"    '{collab['collaboration_name']}' - {revenue_str} | Hype: {collab['hype_score']}/10"
        )
    ...
    for i, brand in enumerate(cultural_impact, 1):
        print(
            f"  #{i} {brand['brand_name']}: {brand['cultural_influence_tier']} (Score: {brand['cultural_impact_score']})"
        )
        print(
            f"      {brand['collaboration_count']} collaborations | {brand['critical_moments']} critical moments"
        )
    ...
    for personality in personalities:
        values_str = (
            ", ".join(personality["brand_values"]) if personality["brand_values"] else "N/A"
        )
    ...
    for fin in financials:
        growth_str = (
            f"+{fin['growth_rate_percentage']}%"
            if fin["growth_rate_percentage"] and fin["growth_rate_percentage"] > 0
            else f"{fin['growth_rate_percentage']}%" if fin["growth_rate_percentage"] else "N/A"
        )
        margin_str = (
            f"{fin['profit_margin_percentage']}%" if fin["profit_margin_percentage"] else "N/A"
        )
    ...
    tables_info = [
        (
            "core.brands",
            "Extended with 25+ new fields including founder, HQ, financials, story, mission",
        ),
        ...
        (
            "core.brand_financials",
            "6 years of financial data with growth and profitability metrics",
        ),
    ]
    ...
    views_info = [
        ...
        (
            "analytics.brand_collaboration_network",
            "Partnership analysis with success and hype metrics",
        ),
        ("analytics.brand_innovation_timeline", "Technology and innovation milestone tracking"),
        (
            "analytics.brand_financial_evolution",
            "Multi-year financial performance and growth analysis",
        ),
        (
            "analytics.brand_personality_analysis",
            "Brand values, personality traits, and sustainability",
        ),
