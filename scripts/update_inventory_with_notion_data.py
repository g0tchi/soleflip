...
def update_item_via_sql_simulation(item, notion_data):
    ...
    print(
        f"  Net Purchase Price: {net_purchase_price:.2f} EUR (was gross: {notion_data['gross_buy']:.2f} EUR)"
    )
    print(f"  Sale Price: {sale_price:.2f} EUR")
    print(
        f"  Calculated Shelf Life: {shelf_life_days} days (expected: {notion_data['expected_shelf_life']})"
    )
    ...
    if not shelf_life_accurate:
        print(
            f"    [WARNING] Shelf life mismatch: {shelf_life_days} vs {notion_data['expected_shelf_life']}"
        )
    if not roi_accurate:
        print(
            f"    [WARNING] ROI mismatch: {roi_percentage:.1f}% vs {notion_data['expected_roi']:.1f}%"
        )
...
def test_bi_api_with_real_item(item_id):
    ...
    try:
        response = requests.get(
            f"{API_BASE}/api/analytics/business-intelligence/inventory/{item_id}/analytics"
        )
        if response.status_code == 200:
    ...
    try:
        response = requests.post(
            f"{API_BASE}/api/analytics/business-intelligence/inventory/{item_id}/update-analytics"
        )
        if response.status_code == 200:
...
def main():
    ...
    if total_items > 0:
        avg_calculated_shelf_life = (
            sum(item["calculated_shelf_life"] for item in updated_items) / total_items
        )
        avg_expected_shelf_life = (
            sum(item["expected_shelf_life"] for item in updated_items) / total_items
        )
