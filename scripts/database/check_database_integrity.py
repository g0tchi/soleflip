...
async def check_database_integrity():
    ...
    original_brands = await conn.fetchval('SELECT COUNT(*) FROM core.brands WHERE founder_name IS NULL')
    enriched_brands = await conn.fetchval('SELECT COUNT(*) FROM core.brands WHERE founder_name IS NOT NULL')
    ...
    original_examples = await conn.fetch('SELECT name, category, segment FROM core.brands WHERE founder_name IS NULL LIMIT 5')
    for brand in original_examples:
        print(f'  {brand["name"]} - {brand["category"]} - {brand["segment"]}')
    
    print('\nBEISPIELE ERWEITERTER BRANDS:')  
    enriched_examples = await conn.fetch('SELECT name, founder_name, headquarters_city FROM core.brands WHERE founder_name IS NOT NULL LIMIT 5')
    for brand in enriched_examples:
        print(f'  {brand["name"]} - Gegründet von: {brand["founder_name"]} in {brand["headquarters_city"]}')
    ...
    try:
        orphaned_check_queries = [
            ('brand_history -> brands', 'SELECT COUNT(*) FROM core.brand_history WHERE brand_id NOT IN (SELECT id FROM core.brands)'),
            ('brand_attributes -> brands', 'SELECT COUNT(*) FROM core.brand_attributes WHERE brand_id NOT IN (SELECT id FROM core.brands)'),
            ('brand_financials -> brands', 'SELECT COUNT(*) FROM core.brand_financials WHERE brand_id NOT IN (SELECT id FROM core.brands)'),
        ]
