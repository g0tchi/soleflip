...
def main():
    ...
    checks = [
        ...
        # Code quality tools
        (lambda: run_command_check(["python", "-m", "black", "--version"], "Black formatter"), "Black"),
        (lambda: run_command_check(["python", "-m", "isort", "--version"], "isort import sorter"), "isort"),
        (lambda: run_command_check(["python", "-m", "flake8", "--version"], "Flake8 linter"), "Flake8"),
        
        # Testing tools
        (lambda: run_command_check(["python", "-m", "pytest", "--version"], "Pytest test framework"), "Pytest"),
        
        # Database tools
        (lambda: run_command_check(["python", "-m", "alembic", "--version"], "Alembic migrations"), "Alembic"),
        
        # FastAPI import test
        (lambda: run_command_check(["python", "-c", "from main import app; print('FastAPI OK')"], "FastAPI application"), "FastAPI"),
        
        # GitHub workflows
        (lambda: check_file_exists(".github/workflows/ci.yml", "CI workflow"), "CI Workflow"),
        (lambda: check_file_exists(".github/workflows/deploy.yml", "Deploy workflow"), "Deploy Workflow"),
        (lambda: check_file_exists(".github/workflows/dependencies.yml", "Dependencies workflow"), "Deps Workflow"),
        ...
        # Migration files
        (lambda: check_file_exists("migrations/versions/2025_08_30_1000_add_performance_indexes.py", "Performance indexes migration"), "Performance Migration"),
        (lambda: check_file_exists("migrations/versions/2025_08_30_1030_create_auth_schema.py", "Auth schema migration"), "Auth Migration"),
        ...
        # Deployment scripts
        (lambda: check_file_exists("scripts/deployment/generate_secrets.py", "Secrets generator"), "Secrets Script"),
        (lambda: check_file_exists("scripts/database/migrate_with_backup.py", "Migration script"), "Migration Script"),
    ]
