...
class ProductDiscoveryService:
    ...
    async def discover_products(self):
        ...
        for notion_page in mock_notion_sales:
            ...
            product_record = {
                ...
                "stockx_url": (
                    f"https://stockx.com/{stockx_info.get('url_key')}"
                    if stockx_info and stockx_info.get("url_key")
                    else ""
                ),
                "brand_mismatch": (
                    "Yes"
                    if stockx_info
                    and product_info["notion_brand"].lower() != stockx_info.get("brand", "").lower()
                    else "No"
                ),
            }
    ...
    def export_to_csv(self, filename: str = "products_to_create.csv"):
        ...
        print(
            f"Found on StockX:                 {found_on_stockx} ({found_on_stockx/len(self.products)*100:.1f}%)"
        )

        not_found = len(self.products) - found_on_stockx
        print(
            f"NOT found on StockX:             {not_found} ({not_found/len(self.products)*100:.1f}%)"
        )
        ...
        if not_found > 0:
            ...
            for product in not_found_list[:10]:
                print(
                    f"  • {product['sku']} ({product['notion_brand']}) - Sale: {product['notion_sale_id']}"
                )
        ...
        if brand_mismatches > 0:
            ...
            for product in mismatch_list[:10]:
                print(
                    f"  • {product['sku']}: Notion={product['notion_brand']} vs StockX={product['stockx_brand']}"
                )
