...
class NotionPostgresSyncAnalyzer:
    ...
    def find_matches(self) -> Dict[str, List]:
        ...
        for notion_item in example_notion_data:
            ...
            for postgres_item in self.postgres_data:
                ...
                if self.match_by_stockx_id(
                    notion_item.get("sale_id", ""), postgres_item.get("external_id", "")
                ):
                    score += 50
                    match_reasons.append("stockx_id_match")

                # SKU matching (high priority)
                if self.match_by_sku(notion_item.get("sku", ""), postgres_item.get("sku", "")):
                    score += 30
                ...
                if notion_item.get("brand", "").lower() == postgres_item.get("brand", "").lower():
                    score += 10
        ...
        unmatched_postgres = [
            item for item in self.postgres_data if item["id"] not in matched_postgres_ids
        ]

        unmatched_notion = [
            item for item in example_notion_data if item.get("sku", "") not in matched_notion_ids
        ]
        ...
        return {
            ...
            "match_rate": (
                len(matches) / len(example_notion_data) * 100 if example_notion_data else 0
            ),
        }
    ...
    async def analyze_sync_potential(self):
        ...
        for i, match in enumerate(results["matches"][:5]):
            ...
            logger.info(
                f"  {i+1}. Notion SKU: {notion.get('sku')} <-> PostgreSQL SKU: {postgres.get('sku')}"
            )
            logger.info(
                f"     Price: EUR{notion.get('gross_sale')} <-> EUR{postgres.get('sale_price')}"
            )
