...
class NotionSaleSyncService:
    ...
    async def check_if_already_synced(self, session, order_number: str) -> bool:
        ...
        result = await session.execute(
            select(StockXOrder).where(StockXOrder.stockx_order_number == order_number)
        )
    ...
    async def parse_notion_sale(self, notion_page: dict) -> dict:
        ...
        return {
            ...
            "profit": (
                Decimal(str(profit)) if profit else (Decimal(str(net_sale)) - Decimal(str(net_buy)))
            ),
    ...
    async def sync_sale_to_db(self, session, sale_data: dict) -> bool:
        ...
        try:
            ...
            if not brand:
                brand = Brand(name=sale_data["brand_name"], slug=sale_data["brand_name"].lower())
                session.add(brand)
                await session.flush()

            # 3. Get or create category (assume Sneakers for now)
            result = await session.execute(select(Category).where(Category.name == "Sneakers"))
    ...
    async def bulk_sync_from_notion(self):
        ...
        async with self.db_manager.get_session() as session:
            for notion_page in mock_notion_sales:
                ...
                if await self.check_if_already_synced(session, sale_data["stockx_order_number"]):
                    logger.info(
                        f"Sale already synced, skipping: {sale_data['stockx_order_number']}"
                    )
        ...
        success_rate = (
            (self.stats["newly_synced"] / self.stats["total_found"] * 100)
