...
from shared.database.models import (
    Supplier, InventoryItem, Order, Brand, Category, Product, Size
)
...
class NotionPostgresSyncService:
    ...
    def parse_notion_properties(self, properties: dict, page_url: str) -> Optional[Dict]:
        ...
        try:
            ...
            delivery_date = datetime.fromisoformat(delivery_date_str).date() if delivery_date_str else None
    ...
    async def check_if_synced(self, session, sale_id: str) -> bool:
        ...
        result = await session.execute(
            select(Order).where(Order.stockx_order_number == sale_id)
        )
    ...
    async def get_or_create_product(self, session, sale_data: Dict, brand: Brand, category: Category) -> Product:
    ...
    async def sync_sale(self, session, sale_data: Dict) -> bool:
        ...
        try:
            ...
            inventory = InventoryItem(
                ...
                purchase_date=datetime.combine(sale_data['buy_date'], datetime.min.time()) if sale_data['buy_date'] else None,
                delivery_date=datetime.combine(sale_data['delivery_date'], datetime.min.time()) if sale_data['delivery_date'] else None,
                status='sold',
                external_ids={
            ...
            order = Order(
                ...
                sold_at=datetime.combine(sale_data['sale_date'], datetime.min.time()) if sale_data['sale_date'] else None,
                ...
                raw_data={
                    'notion_page_url': sale_data['notion_page_url'],
                    'pas_metric': float(sale_data['profit'] / sale_data['shelf_life_days']) if sale_data['shelf_life_days'] > 0 else 0,
