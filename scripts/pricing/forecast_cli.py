...
from domains.analytics.services.forecast_engine import (
    ForecastEngine, ForecastConfig, ForecastModel, 
    ForecastHorizon, ForecastLevel
)
...
class ForecastCLI:
        ...
        try:
            if not args.run_id:
                ...
                for model_key, records in model_metrics.items():
                    ...
                    print(f"   MAPE: {latest_record.mape_score:.2f}%" if latest_record.mape_score else "   MAPE: N/A")
                    print(f"   RMSE: {latest_record.rmse_score:.2f}" if latest_record.rmse_score else "   RMSE: N/A")
                    print(f"   MAE:  {latest_record.mae_score:.2f}" if latest_record.mae_score else "   MAE: N/A")
                    print(f"   R²:   {latest_record.r2_score:.4f}" if latest_record.r2_score else "   R²: N/A")
                ...
                for entity_id, entity_forecasts_list in list(entity_forecasts.items())[:5]:  # Show first 5
                    total_forecast = sum(f.forecasted_units for f in entity_forecasts_list)
                    avg_confidence = sum(f.confidence_lower or 0 for f in entity_forecasts_list) / len(entity_forecasts_list)
                ...
                print(f"   Use: forecast_cli accuracy --run-id {run_id} --actual-data sales_data.csv")
    ...
    async def list_forecasts(self, args):
        ...
        try:
            ...
            for entity_id, entity_forecasts in list(entity_groups.items())[:args.limit if args.limit else 10]:
                ...
                avg_confidence = sum((f.confidence_lower or 0 + f.confidence_upper or 0) / 2 
                                   for f in entity_forecasts) / len(entity_forecasts)
                
                print(f"\n🎯 {entity_name}")
                print(f"   ID: {entity_id}")
                print(f"   Model: {entity_forecasts[0].model_name}")
                print(f"   Period: {entity_forecasts[0].forecast_date} to {entity_forecasts[-1].forecast_date}")
                ...
                if args.show_daily and len(entity_forecasts) <= 7:
                    ...
                    for forecast in entity_forecasts:
                        print(f"     {forecast.forecast_date}: {forecast.forecasted_units:.1f} units")
    ...
    async def demand_analysis(self, args):
        ...
        try:
            ...
            for period_type, summary in pattern_summary.items():
                ...
                avg_velocity = summary['avg_velocity'] / pattern_count if summary['avg_velocity'] > 0 else 0
                ...
                if args.show_recent:
                    recent_patterns = sorted(summary['patterns'], 
                                           key=lambda p: p.pattern_date, reverse=True)[:5]
                    print(f"   Recent Patterns:")
                    for pattern in recent_patterns:
                        seasonality = f" (Season: {pattern.seasonality_factor:.2f})" if pattern.seasonality_factor else ""
                        print(f"     {pattern.pattern_date}: {pattern.demand_score:.4f}{seasonality}")
    ...
    async def _display_forecast_results(self, results, args):
        ...
        for i, result in enumerate(results[:args.result_limit if args.result_limit else 5], 1):
            ...
            avg_confidence = sum(pred['confidence_upper'] - pred['confidence_lower'] 
                               for pred in result.predictions) / len(result.predictions)
    ...
    async def _export_forecast_results(self, results, filename, format_type):
        ...
        if format_type == 'json':
            ...
            with open(filename, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    'entity_id', 'entity_type', 'model_name', 'forecast_date',
                    'forecasted_units', 'forecasted_revenue', 'confidence_lower', 'confidence_upper'
                ])
...
def create_parser():
    ...
    gen_parser.add_argument('--horizon', required=True,
    ...
    gen_parser.add_argument('--prediction-days', type=int, default=30, help='Number of days to forecast')
    gen_parser.add_argument('--confidence-level', type=float, default=0.95, help='Confidence level for intervals')
    gen_parser.add_argument('--min-history-days', type=int, default=90, help='Minimum historical data required')
    ...
    gen_parser.add_argument('--export-format', choices=['json', 'csv'], default='json', help='Export format')
    ...
    acc_parser.add_argument('--level', choices=['product', 'brand', 'category'], help='Filter by forecast level')
    acc_parser.add_argument('--horizon', choices=['daily', 'weekly', 'monthly'], help='Filter by horizon')
    ...
    list_parser.add_argument('--level', required=True,
                            choices=['product', 'brand', 'category', 'platform'],
                            help='Forecast level to list')
    list_parser.add_argument('--horizon', required=True,
                            choices=['daily', 'weekly', 'monthly'],
    ...
    demand_parser.add_argument('--level', required=True,
                              choices=['product', 'brand', 'category'],
                              help='Analysis level')
    ...
    backfill_parser = subparsers.add_parser('backfill', help='Generate historical forecasts for validation')
    backfill_parser.add_argument('--model', required=True,
                                choices=['linear_trend', 'seasonal_naive', 'random_forest', 'ensemble'],
                                help='Forecasting model')
    backfill_parser.add_argument('--horizon', required=True,
                                choices=['daily', 'weekly', 'monthly'],
                                help='Forecast horizon')
    backfill_parser.add_argument('--level', required=True,
                                choices=['product', 'brand', 'category'],
                                help='Forecast level')
    ...
    backfill_parser.add_argument('--prediction-days', type=int, default=7, help='Forecast period length')
