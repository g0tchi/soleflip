    ...
    async def calculate_price(self, args):
        ...
        try:
            ...
            if args.product_id:
                ...
                query = select(Product).where(Product.id == product_id).options(selectinload(Product.brand))
                ...
                query = select(Product).where(Product.sku == args.sku).options(selectinload(Product.brand))
                result = await self.db_session.execute(query)
                product = result.scalar_one_or_none()
            ...
            if args.include_products:
                ...
                for i, product in enumerate(top_products, 1):
                    ...
                    if 'total_volume' in product:
                        print(f"     Volume: {product['total_volume']} | Avg Price: €{product['avg_sale_price']:.2f}")
                    elif 'growth_percent' in product:
                        print(f"     Growth: {product['growth_percent']:.1f}% | €{product['first_price']:.2f} → €{product['latest_price']:.2f}")
...
def create_parser():
    ...
    calc_parser.add_argument('--condition', default='new', help='Item condition (new, excellent, etc.)')
    ...
    calc_parser.add_argument('--save-price', action='store_true', help='Save calculated price to database')
    ...
    rules_parser.add_argument('--rule-type', required=True, choices=['cost_plus', 'market_based', 'competitive'], help='Rule type')
    rules_parser.add_argument('--priority', type=int, help='Rule priority (lower = higher priority)')
    ...
    rules_parser.add_argument('--condition-multipliers', help='JSON object with condition multipliers')
    rules_parser.add_argument('--seasonal-adjustments', help='JSON object with seasonal adjustments')
    ...
    perf_parser.add_argument('--metric', choices=['volume', 'price_growth'], default='volume', help='Performance metric')
    
    # Market Analysis Command
    market_parser = subparsers.add_parser('market-analysis', help='Analyze market prices and competition')
