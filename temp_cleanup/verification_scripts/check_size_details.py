...
async def check_size_details():
    ...
    try:
        ...
        if size_details:
            ...
            # Check what products use this size
            products_using_size = await conn.fetch(
                """SELECT DISTINCT p.name, p.sku, b.name as brand_name, pc.name as product_category
                   FROM products.inventory ii
                   JOIN products.products p ON ii.product_id = p.id
                   LEFT JOIN products.brands b ON p.brand_id = b.id
                   LEFT JOIN core.categories pc ON p.category_id = pc.id
                   WHERE ii.size_id = $1
                   LIMIT 10""",
                '7aeaa0e4-a527-4c7f-8dc9-938a0f139924'
            )
            ...
            if products_using_size:
                ...
                for product in products_using_size:
                    print(f"  - {product['name']} (SKU: {product['sku']}) - {product['brand_name']} - Category: {product['product_category']}")
